        ...
        <FieldWrapper>
            ...
            <div className={'k-form-field-wrap'}>
                <Slider
                    ...
                    {...others}
                >
                    {data.map((value: React.Key | null | undefined) => {
                        const numeric = Number(value as any);
                        const position = Number.isFinite(numeric) ? (numeric as number) : 0;
                        const title = value != null ? String(value) : '';
                        const key = value != null ? value : position;
                        return (
                            <SliderLabel title={title} key={key} position={position}>
                                {title}
                            </SliderLabel>
                        );
                    })}
            ...
            <div className={'k-form-field-wrap'}>
                ...
                >
                    {data.map((value: React.Key | null | undefined) => {
                        ...
                        return (
                            <SliderLabel key={value} position={Number.isFinite(numericValue) ? numericValue : 0}>
...
export const FormDropDownTree = (fieldRenderProps: FieldRenderProps) => {
    ...
    const onExpandChange = React.useCallback(
        (event: { item: object; }) => setExpanded(expandedState(event.item, dataItemKey, expanded)),
